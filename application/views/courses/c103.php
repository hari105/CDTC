    ...
    <div>
      ...
      <?php if($c103 != NULL) {   //change for new course
        ...
        <table class="table table-striped table-bordered container">
          ...
          <?php foreach ($c103 as $c) {
            echo '<tr><td>' . $studentCount . '</td><td>' . $c->rollNum . '</td><td>' . $c->username . '</td><td>' . $c->enrolledDate . '</td>
            <td>

            
          <a class = "btn btn-primary" href="' . site_url('Admin/editStudent/' . $c->rollNum) . '/103">Edit</a>
          <button type="button" class ="btn btn-danger" onclick = "delete1(\'' . $c->rollNum . '\',\'103\')">Disenroll</button>
