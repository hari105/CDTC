<?php $this->load->view('common/header.php') ?>
<style>
ul li , ol li{
    color:#fff;
    padding:10px;
}
.carousel-indicators li {
  width: 10px;
  height: 10px;
  border-radius: 100%;
}
</style>

<div class="container course rounded">
        <div class="courseName"> <h1>Course Name : </h1> <h4>Block Chain</h4> 
        <h5 style="color:yellow;"> NULL Club </h5>
        </div>
   
        <a href="<?php echo base_url() ?>index.php/Courses/register" class="btn btn-lg btn-success" value="Register" style="float:right;" >Register</a>
    <div>
    <br>

        <h2>Objective:</h2>
        <ul style="list-style-type:square; " >
            <li>Understand blockchain and distributed ledger systems in a business
environment.</li>
            <li>Learn the important concepts and key use cases of blockchain for
business.</li>
<li>Learn how assets can be transferred in a blockchain network.</li>
        </ul>
<br>

        <h2>Outcome:</h2>
        <ul style="list-style-type:square; " >
            <li>Students will be able to implement block chain principles & practices
in real time</li>
            
        </ul>
<br>

    <h2>Course Content:</h2>
             <ul style="list-style-type:square; " >
            <li><b>Blockchain Overview</b> : Introduction to Blockchain, Harnessing
Blockchain, Blockchain Vs traditional approach, Scope of Blockchain in
different domains, Practical Applications of Blockchain / Case Studies.
</li>
            <li><b>Blockchain Demystified</b>: Architecture of Blockchain, Different platforms
for building Blockchain applications, Introduction to Hyper ledger,
Introduction to Etherium
</li>
<li><b>Blockchain Programming basics</b>: Blockchain Programming ,Sample
Programmes
</li>
        </ul>
<br>

        <h2>Target Group : </h2> <p>II & III B. Tech I Semester students</p>
<br>

        <h2>Duration :  </h2> <p>12 Hours</p>
<br>

        <h2>Course Coordinator :  </h2> <p> G Lavanya, Asst. Prof., Dept of CSE</p>
<br>

        <h2>Instructors : </h2>
        <ol type="1">
            <li>Bharat Nandan Reddy , III B.Tech,CSE-D</li>
            <li>K . P Nishikanth, II B.Tech,CSE-D</li>
            <li>G Lavanya, Asst. Prof. Dept. of CSE</li>
        </ol>
<br>
        <h2>Industry Partner: </h2> <p>Idea Labs</p>
<br>
        

        </div>
</div>

<?php $this->load->view('common/footer.php') ?>